...
def generate_vehicle(route_no, veh_no):  # route no [1 to 5] veh_no [1 to ..]
    ...
    next_generated_vehicle_step += ngvs_step_parameter
    return
...
def Successive_reject(total_routes, total_vehicles):
    ...
    # finding lowest cvar route
    lowest_cvar = [0, 100000]    # first denote index second denote value
    for i in range(1, K+1):
        if cvar_matrix[K-1][i] != 0:
            if cvar_matrix[K-1][i] < lowest_cvar[1]:
                lowest_cvar[0] = i
                lowest_cvar[1] = cvar_matrix[K-1][i]
    print('Lowest_cvar_route = %s with value of %s' % (lowest_cvar[0], lowest_cvar[1]))
...
if __name__ == "__main__":
    ...
    global next_generated_vehicle_step  # step at which next vehicle will generated
    global ngvs_step_parameter
    global global_step  # next step going to occur on simulation
    global global_data
    global global_a  # for calculaing CVaR
    global global_m  # Subdivisions of trapezoidal integral
    # intializing iid_delay 2d array
    global iid_delay

    global ngvs_step_parameter
    print('ngvs_step_parameter \t lowest_cvar_route \t lowest_cvar_value ', file=open("successive_reject_route_choice.xml", "w"))
    parameter = [18, 19, 21, 22, 11, 12, 13, 16, 8, 4]
    for i in range(0, 1):
        total_routes = 5
        # total_vehicles = math.ceil(numpy.random.normal(1000, 10, 1))
        total_vehicles = 1000
        next_generated_vehicle_step = 0  # step at which next vehicle will generated
        ngvs_step_parameter = 0
        global_step = 0  # next step going to occur on simulation
        global_data = {}
        global_a = 0.95  # for calculaing CVaR
        global_m = 100  # Subdivisions of trapezoidal integral
        # intializing iid_delay 2d array
        iid_delay = []
        # ngvs_step_parameter = math.ceil(numpy.random.normal(20, 1, 1))
        ngvs_step_parameter = parameter[i]
        print('ngvs_step_parameter = %s' % (ngvs_step_parameter))
        basic_seting()
        ...
        lowest_cvar = Successive_reject(total_routes, total_vehicles)
        print('%s \t %s \t %s ' % (ngvs_step_parameter, lowest_cvar[0], lowest_cvar[1]),   		file=open("successive_reject_route_choice.xml", "a"))
        traci.close()
        sys.stdout.flush()
